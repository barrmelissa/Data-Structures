...
int _size(struct bst_node* node);
int _height(struct bst_node* node);
int _max(int left, int right);

void go_left(struct bst_iterator* iter, struct bst_node* n);
/*
 * helper functions
 *
 *
...
/*int _sum(int check_sum, struct bst_node* node){
  int new_sum = 0;
  if(node == NULL){
  if(check_sum == 0){
  return 1;
  ...
  new_sum = check_sum - node->val;
  return _max(_sum(new_sum, node->left), _sum(new_sum, node->right));
  }
  */

int _sum(int sum, struct bst_node* bst_node){
	...
	//call both right and left at once
	return _sum(sum-bst_node->val, bst_node->left) || _sum(sum-bst_node->val, bst_node->right);
...
int bst_path_sum(int sum, struct bst* bst) {
	...
	//Do I need to add anything else or pass anything else here?
	return _sum(sum, bst->root);
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
	...
	//initalizing the iterator
	//iterator->bst = bst;
	iterator->stack = stack_create();
	//Needs to only go through once	
	go_left(iterator, bst->root);
...
int bst_iterator_next(struct bst_iterator* iter) {
	struct bst_node* popped;


	popped = stack_top(iter->stack);

	popped = stack_pop(iter->stack);

	go_left(iter, popped->right);
	return popped->val;
}
