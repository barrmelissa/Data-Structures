...
// 	copy -> name = malloc(sizeof(student),sizeof(char)*strlen(student->name));
	copy->name = malloc((1+strlen(student->name))*sizeof(char));
...
struct student* create_student_array(int num_students, char** names, int* ids,
    float* gpas) {
		...
		for (j = 0; j < num_students; j++) {
			init_student(&students[j], names[j], ids[j], gpas[j]);			
		} 
...
struct student* find_max_gpa(struct student* students, int num_students) {
	int high, i;
	float higher = students[0].gpa;
	for(i=0; i<num_students; i++){
		if(higher < students[i].gpa){
			high = i;
			higher = students[i].gpa;
		}
	}
...
struct student* find_min_gpa(struct student* students, int num_students) {
  	int low, i;
	float lower = students[0].gpa;
	for(i=0; i<num_students; i++){
		if(lower > students[i].gpa){
		 	low = i;
			lower = students[i].gpa;
		}		
	}
	return &students[low];
